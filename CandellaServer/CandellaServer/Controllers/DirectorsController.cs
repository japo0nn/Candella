...
namespace CandellaServer.Controllers
{
    ...
    public class DirectorsController : ControllerBase
    {
        ...
        public DirectorsController(UserManager<IdentityUser> userManager, RoleManager<IdentityRole> roleManager, SignInManager<IdentityUser> signInManager, AppDBContext context)
        {
        ...
        public async Task Register([FromBody] RegisterViewModel model)
        {
            ...
            await _context.Directors.AddAsync(person);
        ...
        public async Task Login([FromBody] LoginViewModel model)
        {
            ...
            var isInRole = await _userManager.IsInRoleAsync(user, "Director");
            if (!result.Succeeded)
            {
        ...
        [HttpGet]
        [Route("Current")]
        public async Task<DirectorDto> CurrentUser()
        {
            ...
            return Mapper.Map<Director, DirectorDto>(director);
        }

        [Authorize]
        [HttpPut]
        [Route("setUserData")]
        public async Task setUserData([FromBody] DirectorDto directorDto)
        {
            var director = await _context.Directors
                .Include(x => x.IdentityUser)
                .SingleOrDefaultAsync(x => x.IdentityUser.Email == User.ToUserInfo().Username ||
